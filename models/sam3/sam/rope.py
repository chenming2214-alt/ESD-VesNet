...
def reshape_for_broadcast(freqs_cis: torch.Tensor, x: torch.Tensor):
    ...
    assert 0 <= 1 < ndim
    # NOTE:
    # Upstream SAM3 checkpoints often precompute RoPE tables for a fixed resolution (e.g. 1008 => 63x63 tokens).
    # In this repo we sometimes want to run at other square resolutions (e.g. 1024 => 64x64 tokens).
    # When shapes mismatch, fall back to recomputing axial RoPE for the current token grid size.
    if freqs_cis.shape != (x.shape[-2], x.shape[-1]):
        seq_len = int(x.shape[-2])
        # NOTE: `x` is complex-valued after `view_as_complex(..., 2)`, so last dim is `head_dim/2`.
        dim = int(x.shape[-1]) * 2
        side = int(round(seq_len ** 0.5))
        if side * side != seq_len:
            raise AssertionError(f"RoPE freqs_cis mismatch: freqs={tuple(freqs_cis.shape)} x={tuple(x.shape)}; "
                                 f"cannot infer square grid from seq_len={seq_len}")
        freqs_cis = compute_axial_cis(dim=dim, end_x=side, end_y=side, device=x.device)
