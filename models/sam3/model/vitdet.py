...
def reshape_for_broadcast(freqs_cis: torch.Tensor, x: torch.Tensor) -> torch.Tensor:
    ndim = x.ndim
    assert 0 <= 1 < ndim
    # Allow running at square resolutions different from the checkpoint-precomputed RoPE table.
    if freqs_cis.shape != (x.shape[-2], x.shape[-1]):
        seq_len = int(x.shape[-2])
        # NOTE: `x` here is complex-valued after `view_as_complex(..., 2)`,
        # so its last dim is `head_dim/2`.
        dim = int(x.shape[-1]) * 2
        side = int(round(seq_len ** 0.5))
        if side * side != seq_len:
            raise AssertionError(
                f"RoPE freqs_cis mismatch: freqs={tuple(freqs_cis.shape)} x={tuple(x.shape)}; "
